...
const shockTolerance=Math.min(100,Math.max(0,liquidityCoverage*.7+debtProgress*.3));
const resilience=Math.round(liquidity*.4+debtBurden*.3+shockTolerance*.3);
return{balance,savings,debtFreeMonth,years,totalInterest,totalPaid,minSavings,failure,liquidity,debtBurden,debtProgress,shockTolerance,resilience}
}}
...
if(!profile){calculate();if(!profile)return}const model=new FinancialModel(profile),base=model.run("none"),shock=model.run(event),survived=!shock.failure,scoreDelta=shock.resilience-base.resilience;
$("stressResult").innerHTML=`<div class="result-status ${survived?"survived":"failed"}"><div class="status-icon">${survived?"✓":"!"}</div><div><p class="eyebrow">${survived?"PLAN SURVIVES":"PLAN IS AT RISK"}</p><h3>${survived?"Your plan absorbs this scenario.":"This scenario exhausts your modeled financial cushion."}</h3><p>${stressExplanation(event)}</p></div></div><div class="before-after"><div><span>Baseline debt-free</span><b>${base.years===null?"Not reached":base.years.toFixed(1)+" yrs"}</b></div><div><span>Scenario debt-free</span><b>${shock.years===null?"Not reached":shock.years.toFixed(1)+" yrs"}</b></div><div><span>Resilience</span><b>${Math.round(shock.resilience)}/100</b><small>${scoreDelta>=0?"+":""}${Math.round(scoreDelta)} points</small></div></div>`
}
...
setTimeout(()=>{const model=new FinancialModel(profile),o=[];for(let i=0;i<n;i++)o.push(model.run("none",stochasticInputs()));const df=o.filter(x=>x.years!==null),fail=o.filter(x=>x.failure).length,exhausted=o.filter(x=>x.minSavings<=0).length,my=quantile(df.map(x=>x.years),.5);
$("mcDebtFreeProb").textContent=pct(df.length/n*100);$("mcFailureProb").textContent=pct(fail/n*100);$("mcMedianYears").textContent=my===null?"Not reached":my.toFixed(1)+" yrs";$("mcSavingsExhaustion").textContent=pct(exhausted/n*100);renderHistogram(df.map(x=>x.years));
$("mcInterpretation").innerHTML=`<p class="eyebrow">WHAT THIS MEANS</p><h3>${fail/n<=.10?"Your plan is relatively resilient across simulated futures.":fail/n<=.25?"Your plan survives many futures, but has meaningful failure risk.":"Your plan is highly sensitive to economic disruption."}</h3><p>Across ${n.toLocaleString()} futures, <strong>${pct(exhausted/n*100)}</strong> exhausted liquid savings at some point, while <strong>${pct(fail/n*100)}</strong> either exhausted savings or failed to clear debt within the horizon.</p>`;$("mcResults").classList.remove("hidden");$("mcStatus").textContent="Simulation complete. These are modeled possibilities, not predictions."},30)
}
...
function applyTweak(){if(!profile)return;const e=num("tweakEmergency"),d=num("tweakDebt"),before=new FinancialModel(profile).run(),after=new FinancialModel(profile).run("none",null,{emergencyMonths:e,debtAllocation:d});$("tweakResult").innerHTML=`<div><span>Before</span><b>${Math.round(before.resilience)}/100</b><small>Shock tolerance: ${Math.round(before.shockTolerance)}/100</small></div><div><span>After</span><b>${Math.round(after.resilience)}/100</b><small>Shock tolerance: ${Math.round(after.shockTolerance)}/100</small></div><p>${after.resilience>before.resilience?"This change improves modeled baseline resilience.":"This change does not improve the modeled baseline score; try another combination."}</p>`;$("tweakResult").classList.remove("hidden")}
